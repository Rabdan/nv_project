  ...
  <div class="max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-400">Manage generated content — approve, edit, preview and publish.</p>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <UButton color="yellow" variant="soft" size="sm" @click="refresh" :loading="loading">
          Refresh
        </UButton>

        <UButton color="yellow" size="sm" @click="bulkApprove" :disabled="!hasSelection">
          Approve Selected
        </UButton>
        <UButton color="green" size="sm" @click="bulkPublish" :disabled="!hasSelection">
          Publish Selected
        </UButton>
      </div>
    </div>
    ...
    <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
      <div class="p-4 border-b border-gray-700 flex items-center gap-4">
        <UButton size="sm" variant="outline" color="gray" @click="toggleSelectAll">
      ...
      <div v-else class="divide-y divide-gray-700">
        ...
        >
          <div class="flex items-start gap-3 w-full md:w-1/4">
            <input type="checkbox" class="mt-1" :checked="isSelected(post)" @change="toggleSelect(post)" />
            <div class="flex-1">
              <div class="flex items-center gap-2">
                ...
                <div class="text-sm text-white font-medium truncate">{{ post.title || previewText(post) }}</div>
              </div>
              <div class="text-xs text-gray-400 mt-1">{{ formatDate(post.scheduledDate) }}</div>
            </div>
          </div>

          <div class="flex-1">
            <p class="text-sm text-gray-200 leading-snug line-clamp-3">{{ post.content || 'Content not generated yet' }}</p>
            <div class="mt-3 flex items-center gap-2 flex-wrap">
              <UButton size="sm" color="yellow" variant="soft" @click="approvePost(post)" :disabled="post.status === 'approved'">
                Approve
              </UButton>

              <UButton size="sm" color="gray" variant="outline" @click="openEdit(post)">
                Edit
              </UButton>

              <UButton size="sm" color="green" variant="soft" @click="publishPost(post)" :disabled="post.status === 'published'">
                Publish
              </UButton>

              <UButton size="sm" color="gray" variant="ghost" @click="previewPost(post)">
                Preview
              </UButton>

              <span class="ml-auto text-xs px-2 py-1 rounded" :class="statusColor(post.status)">
                {{ post.status }}
              </span>
            </div>
          </div>
    ...
    <UDialog v-model:show="editOpen" :size="'lg'">
      ...
      <template #default>
        <div class="space-y-3">
          ...
          <UInput v-model="editing.scheduledDate" type="datetime-local" />
        </div>
      </template>
      <template #footer>
        <div class="flex justify-end gap-2">
          <UButton variant="ghost" color="gray" @click="editOpen = false">Cancel</UButton>
          <UButton color="yellow" @click="saveEdit">Save</UButton>
        </div>
      </template>
    </UDialog>
  </div>
...
const allSelected = computed(() => posts.value.length > 0 && posts.value.every(p => selected.value.has(p._id)));
...
const bulkApprove = async () => {
  ...
  try {
    ...
    await Promise.all(ids.map(id => n8n.updatePostStatus(id, 'approved')));
    selected.value = new Set();
    await loadPosts();
  } catch (e) {
...
const bulkPublish = async () => {
  ...
  try {
    const ids = Array.from(selected.value);
    await Promise.all(ids.map(id => n8n.updatePostStatus(id, 'published')));
    selected.value = new Set();
    await loadPosts();
  } catch (e) {
...
const openEdit = (post) => {
  ...
  editing.scheduledDate = post.scheduledDate ? formatForInput(post.scheduledDate) : '';
  editOpen.value = true;
};
...
const saveEdit = async () => {
  ...
  try {
    ...
    const idx = (strategy.posts || []).findIndex(p => p._id === editing._id);
    if (idx === -1) throw new Error('Post not found in strategy');

    strategy.posts[idx].content = editing.content;
    if (editing.scheduledDate) strategy.posts[idx].scheduledDate = new Date(editing.scheduledDate).toISOString();
    ...
    if (n8n.saveStrategy) {
      ...
      await n8n.updatePostStatus(editing._id, strategy.posts[idx].status || 'generated');
    }
...
const previewPost = (post) => {
  ...
  if (win) {
    win.document.write('<pre style=\"white-space:pre-wrap;font-family:ui-sans-serif;padding:20px;background:#0b1220;color:#e6edf3;\">' + escapeHtml(body) + '</pre>');
...
const filteredPosts = computed(() => {
  ...
  return posts.value
    .filter(p => {
      ...
      if (!q) return true;
      return (p.content || '').toLowerCase().includes(q) || (p.title || '').toLowerCase().includes(q);
    })
...
const previewText = (post) => {
  ...
  if (post.content) return post.content.slice(0, 60) + (post.content.length > 60 ? '…' : '');
